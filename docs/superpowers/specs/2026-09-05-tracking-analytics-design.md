...
A tela desta fase é a seção **"Visitas nos últimos 7 dias"** de `docs/wireframes/alva-studio-ui-reference.html`, no cartão da coluna esquerda da visão de projeto, logo abaixo de "Conteúdos do projeto". Ela é o consumidor do resumo autenticado e entra nesta fase, não numa posterior. Reproduzi-la é obrigatório e literal, conforme a "Regra de fidelidade visual" do `AGENTS.md`: um cartão `.surface.analytics-card`, sete barras verticais em `div` com altura percentual — sem SVG, sem eixo e sem rótulo numérico —, e a faixa de funil `origem → rota → rota → Lead`. Dois pontos precisam de decisão do dono na homologação: o wireframe rotula a fonte como "Umami · atualizado agora", o que contradiz o coletor interno aprovado na leitura (ii), e o wireframe não desenha estado de carregando, vazio nem erro, que o painel real exige. Os tokens vêm da seção "Biblioteca visual" e já existem em `packages/studio/public/styles.css`; nenhum token novo deve ser criado.
