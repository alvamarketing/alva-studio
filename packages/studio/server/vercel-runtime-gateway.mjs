...
import { signedRuntimeAttribution, REGEX_COOKIE_FBP, FORMATO_FBP } from './runtime-gateway-security.mjs';
...
const allowed=(path)=>path==='/_alva'||path.startsWith('/_alva/')||path.startsWith('/api/public/forms/')||path.startsWith('/api/public/pages/');
...
export function runtimeGatewayArtifacts(files, { publicationId, snapshotHash, environment, runtimeOrigin, runtimeHmacSecret, providers = [], runtimeBootstrap = true } = {}) {
  ...
  const output = files.map((file) => file.file.endsWith('.html')
    ? { ...file, data: runtimeBootstrap ? extendRuntimeCsp(appendRuntimeBootstrap(file.data.replace(absoluteForms, '/api/public/forms/').replace(absolutePages, '/api/public/pages'), scope.publicationId), providers, gatewayOrigin(runtimeOrigin), nonce) : file.data.replace(absoluteForms, '/api/public/forms/').replace(absolutePages, '/api/public/pages') }
    : { ...file });
  const existing = output.find((file) => file.file === 'vercel.json');
  const base = existing ? JSON.parse(existing.data) : { version: 2 };
  const rewrites = [...(base.rewrites || []).filter((rewrite) => !['/_alva/:path*', '/api/public/forms/:path*', '/api/public/pages/:path*'].includes(rewrite.source)), { source: '/_alva/:path*', destination: '/api/_alva/runtime/:path*' }, { source: '/api/public/forms/:path*', destination: '/api/_alva/forms/:path*' }, { source: '/api/public/pages/:path*', destination: '/api/_alva/pages/:path*' }];
