...
export class PublicationRuntimeRepository {
  ...
  async saveManifest({ companyId, projectId, manifest }) {
    ...
    const { rows } = await this.database.query(
      `INSERT INTO publication_runtime_manifests
        (company_id, project_id, environment, publication_id, snapshot_hash, version, policy_version, origin, domain, policy, providers, contents)
       ...
       DO UPDATE SET snapshot_hash=EXCLUDED.snapshot_hash, version=EXCLUDED.version, policy_version=EXCLUDED.policy_version, origin=EXCLUDED.origin, domain=EXCLUDED.domain,
         policy=EXCLUDED.policy, providers=EXCLUDED.providers, contents=EXCLUDED.contents, revoked_at=NULL, updated_at=now()
       RETURNING publication_id, snapshot_hash, version, policy_version, origin, domain, environment, policy, providers, contents, revoked_at`,
      [company, project, environment, manifest.publicationId, manifest.snapshotHash, manifest.version, manifest.policyVersion, manifest.origin, manifest.domain, JSON.stringify(manifest.consent), JSON.stringify(manifest.providers), JSON.stringify(manifest.contents || [])],
    );
    return rows[0];
  }
  ...
  async current({ companyId, projectId, environment, publicationId }) {
    ...
    const { rows } = await this.database.query(
      `SELECT publication_id, snapshot_hash, version, policy_version, origin, domain, environment, policy, providers, contents, revoked_at
  ...
  async currentForOrigin({ publicationId, origin }) {
    ...
    const { rows } = await this.database.query(
      `SELECT company_id, project_id, publication_id, snapshot_hash, version, policy_version, origin, domain, environment, policy, providers, contents, revoked_at
         FROM publication_runtime_manifests
